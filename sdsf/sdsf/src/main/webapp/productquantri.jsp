									...
									<form action="Themsanphamproductquantri" method="post"
										...
										<div class="col-12">
											<label for="inputName" class="form-label">Tên sản
												...
												mục của sản phẩm</label> <label for="categorySelect" class="form-label">Chọn danh mục của sản phẩm</label> 
<select class="form-select" id="categorySelect" name="addDanhmucsanpham" required>
    <option value="" selected disabled>Chọn danh mục</option>
    <% 
    LaydulieuReponsitory ldl1 = new LaydulieuReponsitory();
    List<DanhMuc> danhmuc1 = ldl1.Laythongtidanhmuc();
	for (DanhMuc dm : danhmuc1) { %>
			...
			<div class="modal-content">
				...
				<form onsubmit="handleSave(event)" class="card-body row"
					...
					<label for="categorySelect" class="form-label">Chọn danh
						...
						<% 
    LaydulieuReponsitory ldl2 = new LaydulieuReponsitory();
    List<DanhMuc> danhmuc2 = ldl2.Laythongtidanhmuc();
	for (DanhMuc dm : danhmuc1) { %>
        <option value="<%=dm.getMaDanhmuc()%>"><%=dm.getTenDanhMuc()%></option>
    <% } %>
					</select>
